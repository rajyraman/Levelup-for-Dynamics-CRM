...
module LevelUp {
  export class Grid {
    ...
    quickFindFields() {
      let currentView = this.utility.formDocument.querySelector(
          "span.ms-crm-View-Name"
        ),
        resultsArray: LevelUp.Types.ResultRow[] = [
          { cells: ["Quick Find Attribute"] }
        ],
      ...
      if (entityName) {
        let attributes = "FetchXml",
          entitySetName = this.utility.is2016OrGreater
            ? "savedqueries"
            : "SavedQuerySet";
        ...
        this.utility.fetch(entitySetName, attributes, filter).then(view => {
          ...
          if (this.utility.is2016OrGreater) {
            quickFindFields = Array.from(
              new DOMParser()
                .parseFromString(view[0].fetchxml, "text/html")
    ...
    openGrid() {
      ...
      if (etn) {
        if (!viewId) {
          window.open(
            `${this.utility.clientUrlForParams}etn=${etn}&newWindow=true&pagetype=entitylist`,
            "_blank"
          );
        } else {
          let viewTypeCode;
